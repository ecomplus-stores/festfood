    ...
    setSortOrder (sort) {
      ...
      if (facetElement) {
        ...
        if (typeof window !== 'undefined' && window.bootstrap && window.bootstrap.Collapse) {
          const collapse = window.bootstrap.Collapse.getOrCreateInstance(facetElement)
          collapse.toggle()
        } else {
          // Fallback to simple class toggle
          facetElement.classList.toggle('show')
        }
        
        // Update the icon direction
        const facetTitle = facetElement.previousElementSibling
        if (facetTitle && facetTitle.classList.contains('facet-title')) {
          const icon = facetTitle.querySelector('.facet-icon i')
          if (icon) {
            const isExpanded = facetElement.classList.contains('show') || 
                             facetElement.classList.contains('collapsing')
